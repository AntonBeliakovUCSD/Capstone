...
base_path <- "/Users/gurmandhaliwal/Capstone/data"
setwd(base_path)
...
Regular Regression: 
```{r}
ggplot(summary_df_clean, aes(x = BetaSquared, y = Heritability)) + 
  geom_point(aes(color = Heritability), size = 3) +
...
```
```{r}
model_log <- lm(Heritability ~ BetaSquared, data = summary_df_clean)
summary(model_log)
```
