...
library(bigsnpr)
library(data.table)
setwd("/Users/gurmandhaliwal/Library/CloudStorage/Dropbox/180B")
```
...
snp_data_list <- list()
for (chr in 1:22) {  # Assuming chromosomes 1-22
    #bed_file <- paste0("LDREF/1000G.EUR.", chr, ".bed")
    #snp_data_list[[chr]] <- snp_readBed(bed_file)
    rds_file <- paste0("/Users/gurmandhaliwal/Library/CloudStorage/Dropbox/180B/LDREF/1000G.EUR.", chr, ".rds")
...
ggplot(plot_df, aes(x = CausalVariance)) +
  ...
  scale_x_continuous(limits = c(NA, 3)) + # Set x-axis limits with upper limit at 3
  theme_minimal(base_family = "Helvetica", base_size = 12) +
  theme(plot.background = element_rect(fill = "white", color = NA), # Cute background color
        panel.background = element_rect(fill = "lightgrey", color = NA), # Panel color
